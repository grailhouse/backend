...
module.exports = {
	getLink: async function (shoe, callback) {
		var options = {
			...
			headers: {
				"User-Agent":
					"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_2) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.4 Safari/605.1.15-API",
			},
		};
		request.post(options, function (error, response, data) {
			if (error) {
				let err = new Error(
					"Could not connect to Flight Club while searching '" + shoe.styleID + "' Error: ",
					error
				);
				console.log(err);
				callback(err);
				...
				let err = new Error(
					"Could not connect to Flight Club while searching '" + shoe.styleID + "' -  Status Code: ",
					response.statusCode
				);
				console.log(err);
				callback(err);
				...
				try {
					...
					for (var i = 0; i < json.data.getProductTemplate.newSizes.length; i++) {
						priceMap[json.data.getProductTemplate.newSizes[i].size.display] =
							json.data.getProductTemplate.newSizes[i].lowestPriceOption.price.value / 100;
					}
